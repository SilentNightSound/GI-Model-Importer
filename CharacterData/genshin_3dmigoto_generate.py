...
def main():
    ...
    position, blend, texcoord = collect_vb(args.name, "Head", stride)
    ...
    x,y,z = collect_vb(args.name, "Body", stride)
    position += x
    blend += y
    texcoord += z
    ...
    if extra_flag:
        ...
        x, y, z = collect_vb(args.name, "Extra", stride)
        position += x
        blend += y
        texcoord += z
        ...
        if extra2_flag:
            ...
            x, y, z = collect_vb(args.name, "Extra2", stride)
            position += x
            blend += y
            texcoord += z
...
def collect_vb(name, classification, stride):
    position = bytearray()
    blend = bytearray()
    texcoord = bytearray()
