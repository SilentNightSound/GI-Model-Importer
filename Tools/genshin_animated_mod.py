...
def main():
    ...
    parser.add_argument("-spd", "--speed", type=float, help="Speed of the animation (Maximum 10)")
    ...
    if args.speed:
        interval = int(10/float(args.speed))
    else:
        def is_float(s):
            try:
                float(s)
                ...
                return False
        print("\nPlease enter the speed of the animation (can also enter this with -spd flag, or set later in .ini). Must be lower than 10, can be a decimal\n")
        speed = input()
        while not speed or not is_float(speed) or float(speed) > 10:
            print("Speed invalid, must be a number lower than 10, use dots(.) for decimals\n")
            speed = input()
        interval = int(10/float(speed))
